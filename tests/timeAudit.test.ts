...
describe("describeChange", () => {
  it("summarises edits", () => {
    ...
    expect(describeChange(before, after)).toBe("hours 2h → 3h 30m, note changed");
  });

  it("summarises creates and deletes", () => {
    expect(describeChange(null, snapshot(base))).toBe("2h on 2026-09-24");
    expect(describeChange(snapshot(base), null)).toBe("2h on 2026-09-24");
  });
});
